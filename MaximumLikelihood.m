...
% emitter. If sensor and emitter locations are not provided in assignS and 
% assignE they are randomly placed over the Region of Interest (ROI). This 
% random placement makes sure that sensors and emitters are placed in 
% seperate grids.
%
...
% Distances are estimated from each sensor. The problem is formed as a
% solution to the equation system $y = X b$ where b is the vector with
% unknown emitter location values.

% Find estimated distance from all sensors
estDist = (r./P_E).^(-1/alpha_assumed);

% Creation of $X = [2(x_{N_s} - x_a) 2(y_{N_s} - y_a)]$ where a is all 
% numbers from 1 to $N_s-1$. 
X = zeros(N_s-1,2);
for k = 1:N_s-1
    X(k,1)=2*(sPos(1,N_s)-sPos(1,k));
    X(k,2)=2*(sPos(2,N_s)-sPos(2,k));
end

% Creation of;
% $y = [-{x_a}^2-{y_a}^2+{d_a}^2-(-{x_{N_s}}^2-{y_{N_s}}^2+{d_{N_s}}^2)]$
% where a is all numbers from 1 to $N_s-1$. 
y = zeros(N_s-1,1);
for k = 1:N_s-1
    y(k)= (-sPos(1,k)^2-sPos(2,k)^2+estDist(k)^2 ...
        -(-sPos(1,N_s)^2-sPos(2,N_s)^2+estDist(N_s)^2));
end
solution = X\y;
ex = solution(1);
ey = solution(2);
